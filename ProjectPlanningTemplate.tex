...
Features can also be extracted from these by measuring the time elapsed from a press/release of one key to another.

With these concepts now explained, the related literature can be presented. 
Before the relevant PA literature is described, the CA system to be extended is summarized in \cref{sec:related-CA} in order to allow for further discussions on what PA techniques may fit the CA system.
...
Similarly to Bours' study \cite{BOURS201236}, Mondal's trust model worked by comparing a single key action or digraph (two consequtive key actions) to the genuine user's reference and having the result impact the current \textit{trust level} by means of a penalty-and-reward system.
...
In Mondal's machine learning approach, an \textit{Artificial Neural Network}, a \textit{Counter-Propagation Artificial Neural Network} and a \textit{Support Vector Machine} were combined in a \textit{Multi-Classifier Fusion} architecture \cite{mondal}. 
