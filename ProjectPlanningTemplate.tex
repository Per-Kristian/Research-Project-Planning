...
Therefore, this goal of this project is to extend an existing CA system with a PA system in hopes of eliminating the inherent disadvantages of both systems as well as significantly improving detection rates.
...
Most of the existing PA system need several hundreds to over one thousand keystrokes for every periodic check.
This leaves the imposter with too much freedom to roam before the authentication process is triggered.

These systems also have varying accuracy.
Many systems report their accuracy using False Accept Rate (FAR) and False Reject Rate (FRR).
These rates also vary, but they tend to stay below 1\% FAR and around 5\% FRR.
...
However, if the authentication is triggered at shorter intervals, the genuine is rejected and logged out so frequently that the PA system becomes a nuisance.
...
While many systems and applications relies on login processes for authenticating users, most systems do not perform further authentication to ensure that the current user has not changed since logging in.
...
By achieving positive results in terms of accuracy and performance, this project has the potential to increase the practical usability of free-text keystroke dynamics in industries and sectors where a higher level of information security is needed or desired, such as the health sector or other critical infrastructures.
...
The itention of this master thesis is to provide a new hybrid authentication method for free-text keystroke dynamics with better accuracy than similar research.
The CA system to be extended has an Average Number of Imposter Actions (ANIA) rate of x, and an Average Number of Genuine Actions (ANGA) rate of y [reference].
By incorporating a PA system into the CA system, the aim is to improve the ANIA and ANGA rates while keeping computational costs low enough to be used transparently on computers with the power of an average household laptop.
There is also a possibility of developing a method for periodic authentication by combining techniques from different existing research.
%\\\\
%---------------------
%\\\\
...
The result achieved was an ANGA rating of 16057 and an ANIA rating of 499, with 1.3\% of imposters not being detected.
